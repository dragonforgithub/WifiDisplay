...
public class VideoFormats {
    private static final String TAG = "WifiDisplay"+VideoFormats.class.getSimpleName();
    ...
    public static boolean pickBestFormat(VideoFormats sinkSupported, VideoFormats sourceSupported,
        ...
        for (int i = 0; i < ResolutionType.NUM_RESOLUTION_TYPES; ++i) {
            for (int j = 0; j < 32; ++j) {
                ...
                Log.d(TAG, String.format("type %d, index %d, %s supported: sink[%b], source[%b]", i, j, fc,
                        sinkSupported.isResolutionEnabled(ResolutionType.valueOf(i), j),
                        sourceSupported.isResolutionEnabled(ResolutionType.valueOf(i), j)));
